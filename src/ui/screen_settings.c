...
void setup_screen_settings(lv_screens_info *ui) {
    if (ui->screen_settings != NULL) {
        return;
    }
    ui->screen_settings = lv_obj_create(NULL);
    lv_obj_set_style_text_font(ui->screen_settings, &AlibabaPuHuiTi_Regular_15, 0);
    ...
    lv_label_set_text(ui->menu_main_page_label_llm, LV_CUSTOM_SYMBOL_LLM" 智能体");
    ...
    setup_network_setting(ui); // 创建网络设置页面
    setup_settings_llm(ui);
    setup_settings_reset(ui);
    setup_settings_about(ui);
    ...
    lv_menu_set_load_page_event(ui->menu, ui->menu_main_page_label_llm, ui->menu_llm_page);
    lv_menu_set_load_page_event(ui->menu, ui->menu_main_page_label_about, ui->menu_page_about);
    lv_menu_set_load_page_event(ui->menu, ui->menu_main_page_label_reset, ui->menu_page_reset);
}
